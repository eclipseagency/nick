export interface Dictionary {
  meta: {
    title: string;
    description: string;
  };
  nav: {
    home: string;
    about: string;
    services: string;
    gallery: string;
    book: string;
    contact: string;
    shop: string;
  };
  hero: {
    badge: string;
    line1: string;
    line2: string;
    line3: string;
    subtitle: string;
    cta1: string;
    cta2: string;
    stat1n: string;
    stat1l: string;
    stat2n: string;
    stat2l: string;
    stat3n: string;
    stat3l: string;
  };
  about: {
    badge: string;
    heading1: string;
    heading2: string;
    p1: string;
    p2: string;
    feat1t: string;
    feat1d: string;
    feat2t: string;
    feat2d: string;
    feat3t: string;
    feat3d: string;
    feat4t: string;
    feat4d: string;
    year: string;
    established: string;
  };
  services: {
    badge: string;
    heading1: string;
    heading2: string;
    subtitle: string;
    s1title: string;
    s1tag: string;
    s1desc: string;
    s1f1: string;
    s1f2: string;
    s1f3: string;
    s2title: string;
    s2tag: string;
    s2desc: string;
    s2f1: string;
    s2f2: string;
    s2f3: string;
    s3title: string;
    s3tag: string;
    s3desc: string;
    s3f1: string;
    s3f2: string;
    s3f3: string;
    s4title: string;
    s4tag: string;
    s4desc: string;
    s4f1: string;
    s4f2: string;
    s4f3: string;
  };
  gallery: {
    badge: string;
    heading1: string;
    heading2: string;
    alt1: string;
    alt2: string;
    alt3: string;
    alt4: string;
    alt5: string;
    alt6: string;
  };
  whynick: {
    badge: string;
    heading1: string;
    heading2: string;
    subtitle: string;
    f1t: string;
    f1d: string;
    f2t: string;
    f2d: string;
    f3t: string;
    f3d: string;
    f4t: string;
    f4d: string;
  };
  testimonials: {
    badge: string;
    heading1: string;
    heading2: string;
    t1name: string;
    t1quote: string;
    t2name: string;
    t2quote: string;
    t3name: string;
    t3quote: string;
    t4name: string;
    t4quote: string;
  };
  contact: {
    heading1: string;
    heading2: string;
    subtitle: string;
    cta1: string;
    cta2: string;
    locationLabel: string;
    locationValue: string;
    websiteLabel: string;
    sinceLabel: string;
  };
  footer: {
    desc: string;
    servicesTitle: string;
    s1: string;
    s2: string;
    s3: string;
    s4: string;
    companyTitle: string;
    about: string;
    galleryLink: string;
    bookNow: string;
    store: string;
    rights: string;
    location: string;
  };
  booking: {
    badge: string;
    heading1: string;
    heading2: string;
    subtitle: string;
    step1: string;
    step2: string;
    step3: string;
    step1instruction: string;
    chooseServices: string;
    pricesFor: string;
    selectOneOrMore: string;
    estimatedTotal: string;
    serviceCount: string;
    servicesCount: string;
    back: string;
    continue: string;
    vehicleLabel: string;
    servicesLabel: string;
    totalLabel: string;
    change: string;
    namePh: string;
    phonePh: string;
    notesPh: string;
    paymentMethod: string;
    payOnline: string;
    splitIn4: string;
    splitIn3: string;
    or: string;
    confirmWhatsapp: string;
    perMonth: string;
    // Car categories
    carSmall: string;
    carMedium: string;
    carLarge: string;
    carSuv: string;
    // Service names
    svcPpfFull: string;
    svcPpfFullDesc: string;
    svcPpfFront: string;
    svcPpfFrontDesc: string;
    svcTintFull: string;
    svcTintFullDesc: string;
    svcTintFront: string;
    svcTintFrontDesc: string;
    svcCeramic: string;
    svcCeramicDesc: string;
    svcCeramicInt: string;
    svcCeramicIntDesc: string;
    // Benefits
    b_selfHealing: string;
    b_rockChip: string;
    b_uvYellowing: string;
    b_resaleValue: string;
    b_highImpact: string;
    b_invisibleFilm: string;
    b_selfHealTech: string;
    b_factoryPaint: string;
    b_uvRejection: string;
    b_heatReduction: string;
    b_glareFree: string;
    b_fadeProtection: string;
    b_dashboardHeat: string;
    b_antiGlare: string;
    b_uvProtection99: string;
    b_crystalClear: string;
    b_hydrophobic: string;
    b_mirrorGloss: string;
    b_dustRepellent: string;
    b_easyWash: string;
    b_leatherStain: string;
    b_dashboardUv: string;
    b_spillRepellent: string;
    b_interiorNew: string;
    // WhatsApp message
    waGreeting: string;
    waVehicle: string;
    waServices: string;
    waTotal: string;
    waName: string;
    waPhone: string;
    waNotes: string;
  };
}
